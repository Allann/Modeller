...
{
    internal class Profile : IGenerator
    {
        private readonly Module _module;
        private readonly Model _model;

        public Profile(ISettings settings, Module module, Model model)
        {
        ...
        public IOutput Create()
        {
            ...
            sb.AppendLine("{");
            sb.AppendLine($"{i1}public class {_model.Name.Singular.Value}Profile : Profile");
            sb.AppendLine($"{i1}{{");
            sb.AppendLine($"{i2}public {_model.Name.Singular.Value}Profile()");
            sb.AppendLine($"{i2}{{");
            ...
            return new File { Name = $"{_model.Name.Singular.Value}Profile.cs", Content = sb.ToString(), CanOverwrite = false };
        }
    }
}