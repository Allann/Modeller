...
namespace Core.Convert.Strategies
{
        ...
        : IConvertStrategy
    {
        ...
        public object Convert(Type from, Type to, object value, IFormatProvider provider)
        {
            ...
            return cto == ConvertTypes[cDoubleIndex]
                ? convertible.ToDouble(provider)
                : cto == ConvertTypes[cDecimalIndex]
                ? convertible.ToDecimal(provider)
                : cto == ConvertTypes[cDateTimeIndex]
                ? convertible.ToDateTime(provider)
                : cto == ConvertTypes[cStringIndex]
                ? convertible.ToString(provider)
                : cto == ConvertTypes[cObjectIndex] ? value : convertible.ToType(cto, provider);
        }
        ...
        private static Type GetTrueType(Type to)
        {
            ...
            return to.IsDerivedFromGenericType(typeof(Nullable<>)) ? to.GetGenericArguments()[0] : to;
        }
