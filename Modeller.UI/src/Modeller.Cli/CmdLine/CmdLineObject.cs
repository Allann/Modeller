...
namespace Core.CmdLine
{
    ...
    /// The properties can be any type that can be converted to and from a string</para>
    ...
    public abstract class CmdLineObject : ICmdLineObject
    {
        ...
        public CmdLineObject(CmdLineOptions options)
        {
            ...
            if (Options.Description == null)
            {
                ...
                Options.Description = att == null ? Application.Description : att.Description;
            }
        ...
        private CmdLineOptions CreateOptions()
        {
            ...
            return att != null ? att.CreateOptions() : new CmdLineOptions();
        }
            ...
            InitializeBy(() =>
                             {
                                     ...
                                     !args[0].StartsWith(Options.ArgumentPrefix))
                                 {
                                     ...
                                     {
                                         for (var i = 0; i < argValues.Length && i < DefaultProperties.Length; i++)
                                         {
                                 ...
                                 for (var i = startIndex; i < args.Length; i++)
                                 {
                                     ...
                                     var argName = args[i].Substring(Options.ArgumentPrefix.Length);
                                     if (argName == "")
                                     {
                                         continue;
                                     }
                                     ...
                                     else if (prop.PropertyType.IsArray)
                                     {
                                         try
                                         {
                                             var splitValues = string.Join(Options.ArraySeparator, argValues).Split(new[] { Options.ArraySeparator }, StringSplitOptions.RemoveEmptyEntries);
        ...
        {
            if (Options.AssignmentDelimiter != _space)
            {
        ...
        protected virtual string[] Validate()
        {
            ...
            var results = CoreExtensions.Validate(this);
        ...
        public virtual string GetHelpText(int maxWidth)
        {
            ...
            {
                helpTextBuilder.AppendLine("ERROR: " + _errors[0]);
            ...
            var indent = new string(_space, maxNameWidth);
            ...
            foreach (var prop in Properties)
            {
                ...
                {
                    helpTextBuilder.AppendLine(prop.Description);
                }
                else
                {
                    // If there isn't a description, we need to add a line break.
                    helpTextBuilder.AppendLine(prop.Name);
                }
                if (prop.Required)
                {
                ...
                else if (prop.ShowDefaultValue)
                {
                    ...
                    if (!ConvertEx.IsEmpty(dflt))
                    {
                        if (dflt is Array arr)
                        {
                            ...
                            dflt = dflt.GetType().GetElementType() == typeof(string) ? $"[\"{string.Join("\",\"", strs)}\"]" : $"[{string.Join(", ", strs)}]";
                        }
                ...
                if (prop.PropertyType.IsEnum)
                {
                    ...
                    enumVals.AppendFormat(indent + $"Possible Values: [{string.Join(", ", enumNames)}]");
                    helpTextBuilder.AppendLine(enumVals.ToString());
                }

                foreach (var validator in prop.GetValidationAtts())
                {
                    var message = validator.FormatErrorMessage(prop.Name);
                    helpTextBuilder.AppendLine(indent + message);
                }
                foreach (var validator in prop.Validators)
                {
                    var message = validator.FormatErrorMessage(prop.Name);
        ...
        {
            if (!string.IsNullOrWhiteSpace( Options.WaitArgName))
            {
                var wait = Properties[Options.WaitArgName];
                if (wait == null)
                {
                    throw new CmdLineException($"The Wait property '{Options.WaitArgName}' was not found.");
                }
        ...
        {
            var id = prop.Aliases.Length == 0 ? prop.Name : (object)prop.Aliases[0];

            return !string.IsNullOrEmpty(prop.Usage)
                ? string.Format("{0}{1} <{2}>", Options.ArgumentPrefix, id, prop.Usage)
                : prop.PropertyType == typeof(bool)
                ? string.Format("{0}{1}[-]", Options.ArgumentPrefix, id)
                : string.Format("{0}{1}{2}<{3}>", Options.ArgumentPrefix, id, Options.AssignmentDelimiter, prop.Name);
        }
